        ...
        if let Interaction::Component(component) = interaction {
            ...
            if let Some(package) = package_name {
                if component.data.custom_id.starts_with("add_") {
                    ...
                    temp.get_channel_config(match_ids(component.guild_id.unwrap())).packages.push(package.to_string());
                    temp.save_config();

                    component.create_response(&ctx.http, CreateInteractionResponse::Message(CreateInteractionResponseMessage::new().content(format!("Package {} added!", package)))).await.unwrap();
                } else if component.data.custom_id.starts_with("remove_") {
                    
                    let mut temp = BotConfig::load_config();
                    let config = temp.get_channel_config(match_ids(component.guild_id.unwrap()));
                    
                    if let Some(index) = config.packages.iter().position(|x| *x == package.to_string()) {
                        config.packages.remove(index);
                        temp.save_config();
                    }

                    component.create_response(&ctx.http, CreateInteractionResponse::Message(CreateInteractionResponseMessage::new().content(format!("Package {} removed!", package)))).await.unwrap();
...
async fn display_packages(ctx: &Context, channel_id: u64) {
    for package in &*COMMAND_GROUPS {
        ...
        let embed = CreateEmbed::default()
            ...
            .footer(CreateEmbedFooter::new("Use buttons below to add/remove this package")).to_owned();


        let message = CreateMessage::new();
        let buttons = vec![CreateButton::new(format!("add_{}", package)).label("Add package").style(ButtonStyle::Success),
            CreateButton::new(format!("remove_{}", package)).label("Remove package").style(ButtonStyle::Danger)];

        let _ = ChannelId::new(channel_id).send_message(ctx, message.add_embed(embed).components(vec![CreateActionRow::Buttons(buttons)])).await;
    }
}

async fn display_info(ctx: &Context, channel_id: u64, guild_id:u64, info_message: &Arc<Mutex<Option<Message>>>) {
    let mut config = BotConfig::load_config();
    let config = config.get_channel_config(&match_ids(GuildId::new(guild_id)));
    ...
    let embed = CreateEmbed::default()
        ...
        .footer(CreateEmbedFooter::new("Info updates every 10 seconds")).to_owned();
...
async fn create_or_edit_embed(channel_id: ChannelId, ctx: &Context, message_lock: &Arc<Mutex<Option<Message>>>, embed: CreateEmbed) {
    let mut message_guard = message_lock.lock().await;
    if let Some(message) = &*message_guard {
        // Edit existing message
        if let Err(err) = channel_id.edit_message(&ctx.http, message, EditMessage::new().add_embed(embed)).await {
